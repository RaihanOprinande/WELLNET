...
<aside class="sidebar-nav-wrapper">
    <div class="navbar-logo">
        <a href="{{ url('/admin/dashboard') }}" class="d-flex align-items-center text-decoration-none">
            <img src="{{ asset('assets/images/logo/logowellnet.png') }}" alt="Logo kiri"
                 class="img-fluid me-2" style="max-height: auto; width: auto;" />
    ...
    <nav class="sidebar-nav">
        <ul>
                    ...
                    <a href="{{ url('/admin/dashboard') }}">
                        <span class="icon">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                ...
                                <path d="M8.75 18.3333C12.2376 18.3333 15.1364 15.8128 15.7244 12.4941C15.8448 11.8143 15.2737 11.25 14.5833 11.25H10C9.30966 11.25 8.75 10.6903 8.75 10V5.41666C8.75 4.7263 8.18563 4.15512 7.50586 4.27556C4.18711 4.86357 1.66666 7.76243 1.66666 11.25C1.66666 15.162 4.83797 18.3333 8.75 18.3333Z"/>
            ...
            @elseif ($role === 'admin')
                ...
                {{-- Users --}}
                @php
                    $usersActive = request()->is('admin/akun*') || request()->is('admin/user_setting*');
                @endphp
                <li class="nav-item nav-item-has-children {{ $usersActive ? 'active' : '' }}">
                    <a href="#" class="{{ $usersActive ? '' : 'collapsed' }}" data-bs-toggle="collapse" data-bs-target="#menuUsers"
                    ...
                    <ul id="menuUsers" class="collapse dropdown-nav {{ $usersActive ? 'show' : '' }}">
                        <li><a href="{{ url('/admin/akun') }}" class="{{ request()->is('admin/akun*') ? 'active' : '' }}">Akun Pengguna</a></li>
                        <li><a href="{{ url('/admin/user_setting') }}" class="{{ request()->is('admin/user_setting*') ? 'active' : '' }}">Akun Setting</a></li>
                    </ul>
                </li>

                {{-- Quiz & Psychoeducation --}}
                @php
                    $quizActive = request()->is('admin/tema_quiz*') || request()->is('admin/soal_quiz*') || request()->is('admin/psychoeducation*') || request()->is('admin/log_quiz*');
                @endphp
                <li class="nav-item nav-item-has-children {{ $quizActive ? 'active' : '' }}">
                    <a href="#" class="{{ $quizActive ? '' : 'collapsed' }}" data-bs-toggle="collapse" data-bs-target="#menuQuiz"
                        aria-expanded="{{ $quizActive ? 'true' : 'false' }}">
                        <span class="icon">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                 xmlns="http://www.w3.org/2000/svg">
                                <path
                ...
                <path
                  d="M2.49999 16.6667C2.03976 16.6667 1.66666 17.0398 1.66666 17.5C1.66666 17.9602 2.03976 18.3334 2.49999 18.3334H14.1667C14.6269 18.3334 15 17.9602 15 17.5C15 17.0398 14.6269 16.6667 14.1667 16.6667H2.49999Z" />
                    ...
                    <ul id="menuQuiz" class="collapse dropdown-nav {{ $quizActive ? 'show' : '' }}">
                        <li><a href="{{ route('tema_quiz.index') }}" class="{{ request()->is('admin/tema_quiz*') ? 'active' : '' }}">Tema Quiz</a></li>
                        <li><a href="{{ route('soal_quiz.index') }}" class="{{ request()->is('admin/soal_quiz*') ? 'active' : '' }}">Soal Quiz</a></li>
                        <li><a href="{{ route('psychoeducation.index') }}" class="{{ request()->is('admin/psychoeducation*') ? 'active' : '' }}">Materi Psychoeducation</a></li>
                        <li><a href="{{ route('log_quiz.index') }}" class="{{ request()->is('admin/log_quiz*') ? 'active' : '' }}">Log Quiz User</a></li>
                    </ul>
                </li>

                {{-- Wellbeing --}}
                @php
                    $wellbeingActive = request()->is('admin/log_pelanggaran*');
                @endphp
                <li class="nav-item nav-item-has-children {{ $wellbeingActive ? 'active' : '' }}">
                    <a href="#" class="{{ $wellbeingActive ? '' : 'collapsed' }}" data-bs-toggle="collapse" data-bs-target="#menuWellbeing"
                       aria-expanded="{{ $wellbeingActive ? 'true' : 'false' }}">
                        <span class="icon">
